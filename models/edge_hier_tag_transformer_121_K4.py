...
import torch.nn as nn
import torch.nn.functional as F
from torch_geometric.nn import SAGPooling, SGConv, GCNConv, SAGEConv, EdgePooling, global_mean_pool as gap, global_max_pool as gmp
...
import torch.nn as nn
import torch.nn.functional as F
from torch_geometric.nn import SAGPooling, GCNConv, SAGEConv, TAGConv, PANConv, PANPooling, global_mean_pool as gap, global_max_pool as gmp
# from models.layers import SAGPool


# 3DGCN model
class edge_hier_tag_transformer_121_k4(torch.nn.Module):
    def __init__(self, n_output=1, n_filters=32, embed_dim=128, num_features_xd=78, num_features_xt=25, output_dim=128, dropout=0.1):

        super(edge_hier_tag_transformer_121_k4, self).__init__()
        ...
        self.pool1 = EdgePooling(in_channels=num_features_xd, dropout=0.2, add_to_edge_score=0)

        self.gc2 = TAGConv(num_features_xd, num_features_xd, K=4)
        self.gc_bn2 = nn.BatchNorm1d(num_features_xd)
        self.pool2 = EdgePooling(in_channels=num_features_xd, dropout=0.2, add_to_edge_score=0)

        self.gc3 = TAGConv(num_features_xd, num_features_xd, K=4)
        self.gc_bn3 = nn.BatchNorm1d(num_features_xd)
        self.pool3 = EdgePooling(in_channels=num_features_xd, dropout=0.2, add_to_edge_score=0)
        ...
        self.fc_g1 = torch.nn.Linear(num_features_xd, output_dim)  # 1024
